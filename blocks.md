...
You also need to specify a command to determine the (initial) state of the toggle (`command_state`). When the command outputs nothing, the toggle is disabled, otherwise enabled.  If `command_state` exits with a non-zero exit code, the toggle is not changed *regardless* of whether the command prints output.
